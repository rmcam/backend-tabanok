...
import { UserLevel } from '../entities/user-level.entity'; // Importar la entidad UserLevel
...
describe('LeaderboardService', () => {
  ...
  let mockUserLevelRepository: Partial<UserLevelRepository & { findOne: jest.Mock }>;
  let mockUserAchievementRepository: Partial<UserAchievementRepository & { count: jest.Mock }>;
  let mockUserMissionRepository: Partial<UserMissionRepository & { count: jest.Mock }>;
  let mockUserRewardRepository: Partial<Repository<UserReward> & { count: jest.Mock }>; // Usar Repository<UserReward>
  ...
  describe('getLeaderboard', () => {
    it('should return a sorted leaderboard with calculated scores and ranks', async () => {
      ...
      const mockUserAchievements = [
        { user: { id: 'user1' } }, { user: { id: 'user1' } }, // Alice: 2 achievements
        { user: { id: 'user2' } }, // Bob: 1 achievement
        { user: { id: 'user3' } }, { user: { id: 'user3' } }, { user: { id: 'user3' } }, // Charlie: 3 achievements
      ];

      const mockUserMissions = [
        { user: { id: 'user1' } }, { user: { id: 'user1' } }, { user: { id: 'user1' } }, // Alice: 3 missions
        { user: { id: 'user2' } }, { user: { id: 'user2' } }, // Bob: 2 missions
        { user: { id: 'user3' } }, // Charlie: 1 mission
      ];

      const mockUserRewards = [
        { user: { id: 'user1' } }, // Alice: 1 reward
        { user: { id: 'user2' } }, { user: { id: 'user2' } }, { user: { id: 'user2' } }, { user: { id: 'user2' } }, // Bob: 4 rewards
        { user: { id: 'user3' } }, { user: { id: 'user3' } }, // Charlie: 2 rewards
      ];

      mockUserRepository.find = jest.fn().mockResolvedValue(mockUsers);
      mockUserLevelRepository.findOne = jest.fn().mockImplementation(({ where: { user: { id } } }) => {
        return Promise.resolve(mockUserLevels.find(level => level.user.id === id));
      });
      mockUserAchievementRepository.count = jest.fn().mockImplementation(({ where: { user: { id } } }) => {
        return Promise.resolve(mockUserAchievements.filter(ua => ua.user.id === id).length);
      });
      mockUserMissionRepository.count = jest.fn().mockImplementation(({ where: { user: { id } } }) => {
        return Promise.resolve(mockUserMissions.filter(um => um.user.id === id).length);
      });
      mockUserRewardRepository.count = jest.fn().mockImplementation(({ where: { user: { id } } }) => {
        return Promise.resolve(mockUserRewards.filter(ur => ur.user.id === id).length);
      });
      ...
      expect(leaderboard[0]).toEqual({ userId: 'user3', username: 'Charlie', score: 1115, rank: 1 });
      expect(leaderboard[1]).toEqual({ userId: 'user1', username: 'Alice', score: 835, rank: 2 });
      expect(leaderboard[2]).toEqual({ userId: 'user2', username: 'Bob', score: 520, rank: 3 });

      expect(mockUserRepository.find).toHaveBeenCalled();
      expect(mockUserLevelRepository.findOne).toHaveBeenCalledTimes(mockUsers.length);
      expect(mockUserAchievementRepository.count).toHaveBeenCalledTimes(mockUsers.length);
      expect(mockUserMissionRepository.count).toHaveBeenCalledTimes(mockUsers.length);
      expect(mockUserRewardRepository.count).toHaveBeenCalledTimes(mockUsers.length);
    });
    ...
    it('should handle users with no related gamification data', async () => {
      ...
      mockUserRepository.find = jest.fn().mockResolvedValue(mockUsers);
      mockUserLevelRepository.findOne = jest.fn().mockImplementation(({ where: { user: { id } } }) => {
        return Promise.resolve(mockUserLevels.find(level => level.user.id === id) || null);
      });
      ...
      expect(leaderboard[0]).toEqual({ userId: 'user1', username: 'Alice', score: 650, rank: 1 });
      expect(leaderboard[1]).toEqual({ userId: 'user2', username: 'Bob', score: 0, rank: 2 }); // Score is 0 if UserLevel is not found
    ...
    it('should handle errors during data retrieval', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith('Error al obtener la tabla de clasificación:', mockError);
      expect(mockUserRepository.find).toHaveBeenCalled();
  ...
  describe('getUserRank', () => {
    ...
    it('should handle errors when calling getLeaderboard', async () => {
      ...
      await expect(service.getUserRank('user1')).rejects.toThrow('Leaderboard calculation failed');
      expect(service.getLeaderboard).toHaveBeenCalledTimes(1);
    });
  });
});
