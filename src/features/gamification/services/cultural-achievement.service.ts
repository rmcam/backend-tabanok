...
export class CulturalAchievementService {
    ...
    async updateProgress(
        ...
        for (const requirement of progress.progress) {
            const update = updates.find(u => u.type === requirement.requirementType);
            if (update) {
                requirement.currentValue = Math.min(requirement.targetValue, requirement.currentValue + update.value);
            }

            const requirementPercentage = (requirement.currentValue / requirement.targetValue) * 100;
        ...
        progress.percentageCompleted = Math.floor(totalPercentage / progress.progress.length);

        // Verificar hitos
        const previousMilestones = progress.milestones.filter(m => m.isAchieved).length;
        for (const milestone of progress.milestones) {
            if (!milestone.isAchieved && progress.percentageCompleted >= milestone.value) {
                milestone.isAchieved = true;
            }
        }
        const newMilestones = progress.milestones.filter(m => m.isAchieved).length;
        ...
        if (requirementsCompleted === progress.progress.length && !progress.isCompleted) {
