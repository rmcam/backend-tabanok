...
export class ContentSeeder extends DataSourceAwareSeed {
  ...
  async run(): Promise<void> {
    console.log("Running ContentSeeder...");
    ...
    for (const sectionName in estructuraContent.estructura.secciones) {
      ...
      console.log(`Processing section: ${sectionName}, path: ${sectionPath}`);
      ...
      try {
        ...
        console.log(`Successfully read section content from: ${sectionPath}`);
        ...
        console.log(
          `Dedicated file not found for section ${sectionName}. Attempting to use consolidated_dictionary.json`
        );
        ...
        if (contentItems.length === 0) {
          console.log(
            `No relevant entries found in consolidated_dictionary.json for section: ${sectionName}. Skipping.`
          );
        }
      }
      ...
      if (contentItems.length > 0) {
        console.log(
          `Content items found for section: ${sectionName}. Processing...`
        );
        for (const contentData of contentItems) {
          console.log(
            `Checking if content "${contentData.title}" already exists...`
          );
          ...
          if (!existingContent) {
            ...
            if (unity && topic) {
              console.log(
                `Creating content: ${contentData.title}, unity: ${unity.title}, topic: ${topic.title}`
              );
        ...
        console.log(
          `No content items found or processed for section: ${sectionName}. Skipping.`
        );
      }
    }
  }
