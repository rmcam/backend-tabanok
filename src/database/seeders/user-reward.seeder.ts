...
export class UserRewardSeeder extends DataSourceAwareSeed {
  ...
  public async run(): Promise<void> {
    ...
    // Obtener todas las recompensas disponibles una sola vez
    const allAvailableRewards = await rewardRepository.find();

    console.log(`Found ${users.length} users for UserRewardSeeder.`);
    console.log(`Found ${allAvailableRewards.length} available rewards.`);
    ...
    if (allAvailableRewards.length === 0) {
      console.log("Skipping UserRewardSeeder: No rewards found.");
      return;
    }
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      const shuffledRewards = allAvailableRewards.sort( // Usar allAvailableRewards
        () => 0.5 - Math.random()
      );
      const maxRewardsToAssign =
        user.roles.includes(UserRole.ADMIN) ? 30 : user.roles.includes(UserRole.TEACHER) ? 20 : 15; // Usar includes y enums
      ...
      if (shuffledRewards.length > 0) {
        ...
        for (let i = 0; i < numberOfValidRewardsToAssign; i++) {
          ...
          // Verificar si ya existe esta recompensa de usuario antes de crearla
          const existingUserReward = await userRewardRepository.findOne({
              where: { userId: user.id, rewardId: selectedReward.id }
          });
          ...
          if (!existingUserReward) {
              ...
              let consumedAt = null;
                ...
                console.log(`User Reward for user "${user.email}" and reward "${selectedReward.name}" already exists. Skipping.`);
              }
            }
          }
        }
      ...
      console.log(`Seeded ${finalUserRewardsToSeed.length} user reward records (development environment).`); // Log the actual number seeded
    } else {
      console.log('Skipping UserRewardSeeder in production environment.');
    }
